...
// "Readiness vs liveness"). Composition-root glue adapting ShaleRepo's
// mount-readiness accessors onto the http server's ReadinessProber port, so
// internal/http stays storage-agnostic and internal/storage stays
// http-agnostic. Tagged slatedb because *storage.ShaleRepo only exists there.
...
// semantics (0 = no floor, desired == 0 vacuously ready) live in the shale
// predicate, not here.
