...
// Sqlite backend's entry into the backend-agnostic conformance suite. Built by
// the DEFAULT `go test ./...` run (no build tag, no cgo, no external services),
// so the contract is exercised against the production sqlite backend on every
// test run.
...
// sqliteConformRepo unions the two sqlite structs (PasteRepo and KeyGateRepo)
// over one shared *sql.DB so they present as a single conformanceRepo. The
// sqlite backend keeps the key-gate rows in their own repo type; the slatedb /
// shale backends are a single struct and need no such union.
...
func TestConformance_Sqlite(t *testing.T) {
	...
	// cross-quota + cross-family-slug subtests exercise the real interaction.
