...
func TestSiteRepo_QuotaCountsAcrossPastesAndSites(t *testing.T) {
	...
	if err := pastes.InsertWithQuotaCheck(p, 0, cap, now); err != nil {
	...
	err := sites.InsertWithQuotaCheck(s, s.Manifest.DedupedSize(), 0, cap, now)
	...
	if err := pastes.InsertWithQuotaCheck(p2, 0, cap, now); !errors.Is(err, ErrOverUserQuota) {
