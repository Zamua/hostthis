...
func TestRoom_KVRoundTripAndScan(t *testing.T) {
	...
	if _, err := repo.PutValue(room.AppSlug, room.ID, "name", []byte("alice"), 0, now); err != nil {
		t.Fatalf("put: %v", err)
	}
	if _, err := repo.PutValue(room.AppSlug, room.ID, "votes", []byte(`{"a":3}`), 0, now); err != nil {
...
func TestRoom_DeleteIsIdempotent(t *testing.T) {
	...
	if _, err := repo.DeleteValue(room.AppSlug, room.ID, "k", now); err != nil {
...
func TestRoom_WriteToNonexistentRoomIsNotFound(t *testing.T) {
	repo := newRoomTestRepo(t)
	now := time.Now().UTC()
	if _, err := repo.PutValue("app12345", domain.NewRoomID(), "k", []byte("v"), 0, now); !errors.Is(err, ErrNotFound) {
		t.Fatalf("put to missing room = %v, want ErrNotFound", err)
	}
	if _, err := repo.DeleteValue("app12345", domain.NewRoomID(), "k", now); !errors.Is(err, ErrNotFound) {
...
func TestRoom_CrossAppIsolation(t *testing.T) {
	...
	if _, err := repo.PutValue("app11111", id, "k", []byte("app1-data"), 0, now); err != nil {
...
func TestRoom_PerRoomByteCap(t *testing.T) {
	...
	if _, err := repo.PutValue(room.AppSlug, room.ID, "big", atCap, 0, now); err != nil {
		t.Fatalf("value at cap rejected: %v", err)
	}
	// Another byte over the cap is rejected; the prior value stays intact.
	if _, err := repo.PutValue(room.AppSlug, room.ID, "more", []byte("x"), 0, now); !errors.Is(err, ErrRoomDataFull) {
...
func TestRoom_PerRoomKeyCap(t *testing.T) {
	...
	for i := range domain.MaxRoomKeys {
		k := keyName(i)
		if _, err := repo.PutValue(room.AppSlug, room.ID, k, []byte("v"), 0, now); err != nil {
			t.Fatalf("key %d under cap rejected: %v", i, err)
		}
	}
	if _, err := repo.PutValue(room.AppSlug, room.ID, "overflow", []byte("v"), 0, now); !errors.Is(err, ErrRoomDataFull) {
...
func TestRoom_PerAppAggregateCap(t *testing.T) {
	...
	if _, err := repo.PutValue(r1.AppSlug, r1.ID, "k", []byte("123456"), appCap, now); err != nil {
	...
	if _, err := repo.PutValue(r2.AppSlug, r2.ID, "k", []byte("78901"), appCap, now); !errors.Is(err, ErrAppRoomsFull) {
