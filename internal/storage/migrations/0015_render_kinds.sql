-- Add 'mermaid', 'pdf', 'csv' and 'json' to the allowed content kinds.
--
-- SQLite cannot ALTER a CHECK constraint in place, so both the pastes table
-- and the versions table are rebuilt with the relaxed constraint, exactly as
-- 0013 did for 'diff'. Column order is preserved so `INSERT ... SELECT *`
-- copies line up positionally.
--
-- DATA-LOSS HAZARD this guards against (same as 0013): the migrator opens the
...
CREATE TABLE pastes_new (
    ...
    kind          TEXT NOT NULL CHECK (kind IN ('html', 'markdown', 'diff', 'mermaid', 'pdf', 'csv', 'json')),
