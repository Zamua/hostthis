-- Add 'log' and 'text' to the allowed content kinds.
--
-- SQLite cannot ALTER a CHECK constraint in place, so both tables are rebuilt
-- with the relaxed constraint, exactly as 0013 and 0017 did. Column order is
-- preserved so `INSERT ... SELECT *` copies line up positionally.
--
-- DATA-LOSS HAZARD this guards against (same as 0017): the migrator opens the
...
CREATE TABLE pastes_new (
    ...
    kind          TEXT NOT NULL CHECK (kind IN ('html', 'markdown', 'diff', 'mermaid', 'pdf', 'csv', 'json', 'flamegraph', 'log', 'text')),
