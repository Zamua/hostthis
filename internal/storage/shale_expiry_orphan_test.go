...
// Orphaned expiry-index entry drain, pinned against the REAL shale backend.
//
// An expiry-index entry whose record is ALREADY GONE must be removed by one
// expiry pass. Without that, the entry resurfaces on every pass forever: the
// scan returns its slug, the record delete no-ops on the missing record, and
// nothing ever touches the entry itself.
//
...
func TestShaleSweep_OrphanExpiryIndexEntryDrains(t *testing.T) {
	...
	// An expiry-index entry with NO paste record behind it.
	mustPutRaw(t, repo, storage.LegacyExpiryKeyForTest(expiresAt, orphan), storage.MarkerValueForTest())

	// A genuinely-expired LIVE paste next to it, so one pass exercises both
	// shapes: real deletion and orphan cleanup.
	...
	// The orphan entry was an index cleanup, not a paste deletion.
	if deleted != 1 {
...
// TestShaleSweep_VerbatimLegacyEntryKeysDrain pins that the entry key FORMAT
// (variable-width RFC3339Nano paste ts, fixed-width site ts) never blocks the
// drain: the scan surfaces each entry with IndexRef equal to the stored bytes,
// and processing that reference removes the exact entry. Only holds when the
// entry is reachable by routed mutation; data placed under a different
// sharding than live routing is the sweep's convergence-guard case, pinned at
// the service level.
func TestShaleSweep_VerbatimLegacyEntryKeysDrain(t *testing.T) {
	...
	// Both fixtures reference records that do not exist.
	...
	// Same contract over expiry_sites/.
...
// paste test above: DeleteExpiredRoom removes the OBSERVED entry regardless of
// the missing record, reporting false (an index cleanup, not a room deletion).
// A genuinely-expired LIVE room next to it pins the true-reporting cascade
...
func TestShaleSweep_OrphanRoomExpiryEntryDrains(t *testing.T) {
	...
	// A roomexpiry entry with NO room record behind it.
	...
	// A genuinely-expired LIVE room next to it, so one pass exercises both
	// shapes: real deletion and orphan cleanup.
	live := domain.Room{
		AppSlug:   app,
		ID:        domain.NewRoomID(),
	...
	deleted := 0
	for _, ref := range refs {
		// Every surfaced reference must carry the EXACT observed entry key.
	...
	// The orphan entry was an index cleanup, not a room deletion.
	if deleted != 1 {
